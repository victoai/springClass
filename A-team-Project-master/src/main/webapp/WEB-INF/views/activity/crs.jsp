...
<body>
    ...
    <tr>
    		<c:forEach var="activity" items="${crsList }" varStatus="status">
 ...
 <h3>
    <c:if test="${prev}">
 <span>[ <a href="${path }/activity/crs?page=${startPageNum - 1}">이전</a> ]</span>
</c:if>

<c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
  <span>
   <a href="${path }/activity/crs?page=${num}">${num}</a> 
 </span>
</c:forEach>

<c:if test="${next}">
 <span>[ <a href="${path }/activity/crs?page=${endPageNum + 1}">다음</a> ]</span>
</c:if>
</h3>
</div>
</body>
